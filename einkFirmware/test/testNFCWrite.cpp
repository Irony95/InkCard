...
void setup()
{
  ...
  memLoc = tag.getCCFileLen();

  Serial.println(F("Writing the first NDEF Text record"));
  tag.writeNDEFText("Hello, World!", &memLoc, true, true); // MB=1, ME=1
  ...
  char theText[40];
  if (tag.readNDEFText(theText, 40, 1))
...
void loop()
{
  ...
  memLoc = tag.getCCFileLen();
  tag.writeNDEFText("new message!", &memLoc, true, true);
  ...
  memLoc = tag.getCCFileLen();
  tag.writeNDEFText("Hello World!", &memLoc, true, true);
}
